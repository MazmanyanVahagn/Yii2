...
class PostController extends Controller
{
			...
			if (!empty($duplicate)) {
				foreach ($duplicate as $row) {
					...
					if ($old_description == $new_description){
						$model->duplicateEntry = Yii::t('app', 'Duplicate entry');
					}
				}
			}
            if (empty($model->duplicateEntry) && $model->save()) {
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->autor_id == Yii::$app->user->id) {
            if ($model->save()) {
                return $this->redirect(['view', 'id' => $model->id]);
            }
        }
